...
const Clients = () => {
  ...
  const [formData, setFormData] = useState({
    clientId: '', firstName: '', lastName: '', companyName: '', telephone: '', address: '', gender: 'Male'
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ clientId: '', firstName: '', lastName: '', companyName: '', telephone: '', address: '', gender: 'Male' });
      fetchClients();
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      <div className="card">
        <h2 className="text-xl font-bold mb-4 text-blue-600">Add New Client</h2>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" placeholder="Client ID" className="border p-2 rounded" required
          ...
          <button type="submit" className="btn-primary md:col-span-2 font-bold">Register Client</button>
        </form>
      </div>
      <div className="card">
        <h2 className="text-xl font-bold mb-4 text-blue-600">Client List</h2>
            ...
            <tbody>
              {clients.map((c) => (
                <tr key={c._id} className="table-row-striped">
                  ...
                  <td className="p-2 border">{c.companyName || 'N/A'}</td>
